...
def load_papers(papers_path: str, date_format: str = "%Y-%m-%d") -> pl.DataFrame:
    ...
    """
    papers = (
        pl.read_ndjson(papers_path)
        .with_columns(pl.col("published").str.to_date(format=date_format, exact=False))
    )
    return papers
...
def process_citations(papers: pl.DataFrame, valid_arxiv_ids: set) -> pl.DataFrame:
    ...
    citations = (
        ...
        .with_row_index()
        .with_columns(
            pl.col("reference_contexts").replace(None, "")
        )
        .with_columns(
            pl.col('reference_contexts').hash().cast(pl.Binary).bin.encode("base64").alias('reference_id')
        )
    )
    return citations
...
def generate_paper_embeddings(
    ...
    paper_embeddings = pl.DataFrame([
        papers['arxiv_id'],
        pl.Series(values=embeddings, name='sentence_embedding', dtype=pl.Array(pl.Float32, shape=(embedding_size,)))
    ])
...
def generate_citation_embeddings_batch(
    ...
    for batch_start in range(0, len(citations), batch_size):
        ...
        citation_embeddings = pl.DataFrame([
            pl.Series(
                values=[
                    embeddings[i][k].squeeze().cpu().numpy()
        ...
        citation_embeddings.write_parquet(str(output_path))


def parse_args():
    """Parse command line arguments."""
    parser = argparse.ArgumentParser(
        description="Build embeddings dataset from arxiv papers",
    ...
    parser.add_argument(
        ...
        type=int,
        default=1000,
        help='Batch size for processing citation embeddings'
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default='%Y-%m-%d',
        help='Date format for parsing publication dates'
    )
